...
def do_test(cfg, model, model_type=''):
    ...
    if comm.is_main_process():
        append_ap50_markdown(cfg, results)
...
def append_ap50_markdown(cfg, results):
    domain_order = [
    ...
    result_path = os.path.join("all_outs", "eval_results.md")
    os.makedirs(os.path.dirname(result_path), exist_ok=True)

    exp_name = os.path.basename(os.path.normpath(cfg.OUTPUT_DIR))
    weight_name = os.path.basename(cfg.MODEL.WEIGHTS) if cfg.MODEL.WEIGHTS else "-"
    values = {}
    for dataset_name, results_i in results.items():
        if "bbox" in results_i and "AP50" in results_i["bbox"]:
            values[dataset_name] = results_i["bbox"]["AP50"]

    need_header = not os.path.exists(result_path) or os.path.getsize(result_path) == 0
    with open(result_path, "a", encoding="utf-8") as f:
        if need_header:
            f.write("| experiment | weight | night_sunny | dusk_rainy | night_rainy | daytime_foggy | daytime_clear |\n")
            f.write("|---|---|---:|---:|---:|---:|---:|\n")
        row = [exp_name, weight_name]
        for _, dataset_name in domain_order:
            ap50 = values.get(dataset_name)
            row.append("-" if ap50 is None else f"{ap50:.2f}")
        f.write("| " + " | ".join(row) + " |\n")
    logger.info("AP50 markdown results appended to %s", result_path)
